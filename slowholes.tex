...
\begin{figure}[htp]
  \centering
  \includegraphics[width=0.5\hsize]{electronholeorbits}
...
negative ion charge repells \iffalse electrons, and because electron
holes are accelerated by applied electric fields as if their charge to
mass ratio were equal to that of the electron, the negative ion charge
repells \fi the electron hole that causes them. The result is rapid
...
documented in simulations\cite{Eliasson2004,Zhou2016}\footnote{A
  recent paper\protect{\cite{Mandal2020}} reports Vlasov simulations
...
\begin{figure}[htp]
  ...
  \caption{(a) The ion distribution functions arising for the sum of
    ...
    $0<v_b<2$. The threshold case is marked in thick black for $\psi=0.5$
    and thick red for $\psi=0.02$ . (b) The force coefficient
    $\delta F/\delta x/\psi^2$ as a function of the beam
    velocity for an equally spaced range of (${\rm sech}^4$ shape) electron hole
...
the minimum that allows the persistence of an electron hole. \emph{That} beam
separation is never large enough that the purely
one-dimensional ion-ion mode is more stable than the obliquely
propagating mode (which can happen for large
$|v_b|$\cite{Forslund1970a}); so if the ion distribution is stable by
the one-dimensional analysis, it is stable to all unmagnetized
electrostatic ion-ion modes.
